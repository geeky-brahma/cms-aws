...
inbox.addEventListener("click", (e) => {
    ...
    const url = `http://3.108.249.104:5000/all_complaints?role=${role}&id=${id}`;
    // console.log(url);
...
document.getElementById("add_user").addEventListener("click", (e) => {
    if (sessionStorage.role === 'admin') {
        ...
        registerButton.addEventListener("click", (e) => {
            ...
            xhr.open("POST", "http://3.108.249.104:5000/register_users");
...
document.getElementById("activate_deactivate_user").addEventListener("click", (e) => {
    if (sessionStorage.role === 'admin') {
        ...
        // Define the request URL
        const url = `http://3.108.249.104:5000/all_users`;
        // console.log(url);
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (dropped) {
                        ...
                        activateButton.onclick = function () {
                            ...
                            fetch(`http://3.108.249.104:5000/activate_user/${empId}`, {method : 'POST'})
                                .then((response) => {
                                    if (!response.ok) {
                                        throw new Error(`HTTP error! Status: ${response.status}`);
                                    }
                        ...
                        dropButton.onclick = function () {
                            ...
                            fetch(`http://3.108.249.104:5000/drop_user/${empId}`, { method: 'DELETE' })
                                .then((response) => {
                                    if (!response.ok) {
                                        throw new Error(`HTTP error! Status: ${response.status}`);
                                    }
...
document.getElementById("sent").addEventListener("click", (e) => {
    ...
    // Define the request URL
    const url = `http://3.108.249.104:5000/sent?role=${role}&id=${id}&name=${name}`;
    // console.log(url);
...
document.getElementById("reports").addEventListener("click", (e) => {
    ...
    async function fetchComplaints(e) {
        ...
        try {
            const response = await fetch(`http://3.108.249.104:5000/report?startDate=${startDate}&endDate=${endDate}&status=${status}`);
